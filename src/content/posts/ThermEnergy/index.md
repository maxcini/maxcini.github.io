...
description: "Long duration energy storage: a lifeline for renewable power generation."
...
Now that we can agree Thermal Energy Storage (TES) can fill a market need, let's take a look at how the system actually works. The [image below][13] shows the system broken up into 3 main units: the charging unit, the storage unit, and the discharge unit. 
...
The stainless steel tanks are heavily insulated in glass foam/ceramic fiber with a refractory brick base, resulting in up to a [99% storage efficiency][16]. Note that majority of the efficiency losses stem from the turbine converting heat into electricity. 
...
In the event a molten salt tank were to leak, there is a relatively low impact to the environment due to the salts being [nonflammable and nontoxic][19] by nature. Also, some have described Concentrated Solar Power plants as ["extraterrestrial looking"][18]. I suppose this is another reason why we are building them in the middle of deserts... 
